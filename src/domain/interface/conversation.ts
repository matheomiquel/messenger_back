...
export interface ConversationInterface {
    read({ order, limit, offset }: { order: string, limit: number, offset: number }): Promise<Conversation[]>
    ...
    removeUser({ userId, conversationId }: { userId: number, conversationId: number }): Promise<void>
}