import { ConversationInterface, MessageInterface } from "@domain/interface";
...
export class MessageDomain {
  ...
  constructor({ messageProvider, conversationProvider }:
    { messageProvider: MessageInterface, conversationProvider: ConversationInterface }) {
  ...
  async read({ order, limit, offset }:
    ...
    return this.messageProvider.read({ order, limit, offset });
  }

  async create({ userId, content, conversationId }:
    { userId: number, content: string, conversationId: number }): Promise<Message> {
    const conversationWithUser = await
    this.conversationProvider.getConversationWithUser({ id: conversationId });
    if (conversationWithUser.users.every(((user) => user.id !== userId))) throw await createError({ message: ["you can't write message in this conversation"], status: 403 });
    return this.messageProvider.create({ userId, content, conversationId });
  }

  async update({ id, userId, content }:
    { id: number, userId: number, content: string }): Promise<void> {
